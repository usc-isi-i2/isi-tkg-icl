...
def permute(tokenizer, scores, cur_step, max_step, cur_seq, seqs, dec_cand, end_char):
    if cur_step == max_step or (len(cur_seq) > 0 and end_char in cur_seq[-1]["token"]):
        _cur_seq = (
            cur_seq[:-1].copy() if end_char in cur_seq[-1]["token"] else cur_seq.copy()
        )
        normalized_logit = (
            sum([x["logit"] for x in _cur_seq]) / len(_cur_seq)
            if len(_cur_seq) > 0
            else -math.inf
        )
        seqs.append(
    ...
    for tok in logits_indices[0][:dec_cand]:
        ...
        permute(
            tokenizer, scores, cur_step + 1, max_step, cur_seq, seqs, dec_cand, end_char
        )
        cur_seq.pop()
...
def parse_results(results):
    logprobs = [
        (int(x["text"]), x["probability"]) for x in results if x["text"].isdecimal()
    ]
...
def predict(tokenizer, model, prompt, args):
    ...
    if args.label and "llama" not in args.model:
        ...
        probs_indices = torch.argsort(
            probs, dim=-1, descending=True
        )  # sort probs and get index
        ...
        results = list(sorted(results, key=lambda x: x["probability"], reverse=True))[
            : args.top_k
        ]
        if args.verbose:
            for x in results:
